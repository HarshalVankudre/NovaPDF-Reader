...
`streamWithFallback()` (in `serve.js`) walks the one-entry chain and streams via `streamAnthropic()` (the Anthropic SDK). **Adaptive thinking is enabled** (`thinking: { type: "adaptive" }` with `output_config: { effort: REASONING_EFFORT }`, currently `"high"` — `low`/`medium`/`high`/`xhigh`/`max`), so Opus decides how much to reason before answering; only the final answer is streamed (thinking deltas are never written out), and the visible answer stays terse per `CHAT_SYSTEM`. **Keep `thinking: { type: "adaptive" }` explicit** — Opus 4.8 runs *without* thinking when the field is omitted. Because thinking delays the first visible token, `FIRST_TOKEN_MS` is sized generously. (On Opus 4.8 `budget_tokens` is removed — adaptive thinking + `effort` replaces it. `"high"` is the exam-day setting per Anthropic's guidance — quality over speed/cost for single-shot reasoning; drop to `"medium"` for snappier answers at some quality cost. `xhigh`/`max` target long agentic runs and add latency without helping one-shot Q&A. Thinking counts toward `max_tokens`, so it's set to 32000 for headroom.) With one model there is **no cross-provider outage fallback** — if the Anthropic API is unreachable, the tutor is down. (Earlier versions kept GLM/Gemini + a fallback chain, then Claude Opus 4.8, then Claude Sonnet 5; now back on a single Claude Opus 4.8.)
...
- It is driven from the keyboard: **Ctrl+Alt+Enter** — or holding **d** while pressing **Enter** — asks the tutor (the sandbox keeps plain Ctrl+Enter for Run); typed `:`-commands in the search box switch state — `:ai` toggles the visible controls, `:new` resets the thread, `:fast` toggles text-only (no slide images) vs vision, and `:sql`/`:db` opens the SQL sandbox. (There is only one model — Claude Opus 4.8 — so there is no model-selection command.)
...
2. **Theory (ERM etc.):** type the question (vision is on by default → Opus reads the actual slide images) or paste a screenshot of the exam question. Ctrl+Alt+Enter (or hold d + Enter).
