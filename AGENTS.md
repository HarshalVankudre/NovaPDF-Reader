...
**1. Server — `serve.js`** (Node `http`, no framework). Three jobs:
   ...
   - LLM proxy so keys stay server-side. Custom routes:
     ...
     - `POST /q` — streaming tutor chat (token-by-token), with stall detection (see Providers).
...
**4. SQL sandbox — `assets/sandbox.js` + `assets/sql-util.js`** — `window.SqlSandbox` (a right-side drawer opened with `:sql`; there is no toolbar SQL button). Two engines, auto-selected: the local-MySQL bridge (exam-exact dialect, preferred) or in-browser SQLite via `sql.js` (vendored at `assets/sql-wasm.js` + `assets/sql-wasm.wasm`, loaded lazily and memoized via `ensureSqlite`, fully offline). The importer accepts: a **binary `.sqlite` snapshot file** (detected by magic header → loaded directly via `new SQL.Database(bytes)`, instant, exact — also importable by pasting the file anywhere in the app), a mysqldump/`.sql` script, or CSV-per-table. `assets/sql-util.js` is a pure, Node-testable UMD module that splits statements, strips `CREATE DATABASE`/`USE`, best-effort-converts a mysqldump to SQLite (`toSqlite`), and builds CSV→SQL. On open, the sandbox checks `/sql/filestatus` and **auto-loads `data/exam.sqlite`** if present (the fast exam-day path).
...
`streamWithFallback()` (in `serve.js`) walks the one-entry chain and streams via `streamAnthropic()` (the Anthropic SDK). **Adaptive thinking is enabled** (`thinking: { type: "adaptive", display: "summarized" }` with `output_config: { effort: REASONING_EFFORT }`, currently `"high"` — `low`/`medium`/`high`/`xhigh`/`max`), so the model decides how much to reason before answering; only the final answer is streamed (thinking deltas are never written out), and the visible answer stays terse per `CHAT_SYSTEM`. On Fable 5 thinking is always on (never send `{ type: "disabled" }`); the explicit `adaptive` field is kept so an Opus model set via the `models.opus` override still thinks. Fable requests additionally opt into the server-side refusal fallback (`server-side-fallback-2026-06-01` + `fallbacks: [{ model: "claude-opus-4-8" }]`) so a rare classifier decline is re-served by Opus 4.8 on the same stream. Because thinking can delay the first visible token by minutes on hard tasks, there is no first-token deadline: an attempt is aborted only when the upstream stream goes silent for `STALL_MS` (120 s). `display: "summarized"` exists for that — the summarized thinking deltas are the liveness signal (same thinking and billing as `"omitted"`; the summaries are never forwarded to the browser). While waiting, the notes pane shows an elapsed-seconds counter. (`budget_tokens` is removed — adaptive thinking + `effort` replaces it. `"high"` is the exam-day setting — quality over speed/cost for single-shot reasoning; drop to `"medium"` for snappier answers at some quality cost. Thinking counts toward `max_tokens`, so it's set to 64000 for headroom.) With one model there is **no cross-provider outage fallback** — if the Anthropic API is unreachable, the tutor is down. A failure *before the first token* that looks transient (429/5xx/529, an SSE `overloaded_error`, a dropped connection — see `isTransientError`) is retried once after a short pause; 4xx errors, a stall, and anything after tokens were written are not. When the browser closes the `/q` response (tab closed, `:stop`, `:new`), the server aborts the upstream request so a paid generation doesn't keep running. (Earlier versions kept GLM/Gemini + a fallback chain, then Claude Opus 4.8, then Claude Sonnet 5, then Opus 4.8 again; now Claude Fable 5.)

`CHAT_SYSTEM` is written for Fable 5, which follows the system prompt closely: each rule is stated once, at normal volume, with its reason (e.g. schema names must be exact *because the query runs against that DB*; citations must be provided slides *because the app verifies them*). Keep that register when editing it — shouted MUST/NEVER rules over-apply on current models.
...
- It is driven from the keyboard: **Ctrl+Alt+Enter** — or holding **d** while pressing **Enter** — asks the tutor (the sandbox keeps plain Ctrl+Enter for Run); typed `:`-commands in the search box switch state — `:ai` toggles the visible controls, `:new` resets the thread, `:stop` aborts the streaming answer (the partial text stays, marked "abgebrochen") and drops queued questions, `:fast` toggles text-only (no slide images) vs vision, and `:sql`/`:db` opens the SQL sandbox. (There is only one model — Claude Fable 5 — so there is no model-selection command.)
...
2. **Theory (ERM etc.):** type the question (vision is on by default → the model reads the actual slide images) or paste a screenshot of the exam question. Ctrl+Alt+Enter (or hold d + Enter).
