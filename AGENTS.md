...
**2. Search engine — `assets/search-engine.js`** — dependency-free Okapi BM25 over an inverted index, plus a German-DB intelligence layer (diacritic folding, prefix expansion for the as-you-type feel, light stemming, bounded-edit fuzzy fallback, a bilingual synonym map, and title/coverage/proximity/**ordered-phrase** boosts). UMD module: the same file runs in the browser (`window.SlideSearchEngine`) and under Node (for the test). A full ranked query over 317 slides (incl. 40 snippets) is ~0.2 ms — per-slide positioned tokens are cached (`_posTokens`) and scoring uses reusable typed-array scratch buffers; keep it that way, snippet re-tokenization used to be ~90% of query time. Tune relevance via `SYN_GROUPS` (synonym/concept bridges) and the BM25 constructor options (`k1`, `b`, `titleBoost`, `coverageWeight`, `proximityWeight`, `phraseWeight`) near the top of the file. `_phrase()` rewards query words appearing in the typed order (a contiguous in-order run); `highlightHTML()` wraps matched words in `<mark>` for snippets. SQL keywords that are also stopwords (e.g. `BETWEEN`) are kept searchable.
...
**4. SQL sandbox — `assets/sandbox.js` + `assets/sql-util.js`** — `window.SqlSandbox` (a right-side drawer opened with `:sql`; there is no toolbar SQL button). Two engines, auto-selected: the local-MySQL bridge (exam-exact dialect, preferred) or in-browser SQLite via `sql.js` (vendored at `assets/sql-wasm.js` + `assets/sql-wasm.wasm`, loaded lazily and memoized via `ensureSqlite`, fully offline). The importer accepts: a **binary `.sqlite` snapshot file** (detected by magic header → loaded directly via `new SQL.Database(bytes)`, instant, exact — also importable by pasting the file anywhere in the app), a mysqldump/`.sql` script, or CSV-per-table. `assets/sql-util.js` is a pure, Node-testable UMD module that splits statements, strips `CREATE DATABASE`/`USE`, best-effort-converts a mysqldump to SQLite (`toSqlite`), and builds CSV→SQL (`,`/`;`/TAB delimiter auto-detected; columns typed INTEGER/REAL/TEXT from the data so numeric comparisons and ORDER BY work — leading-zero values stay TEXT, a `;`-file's decimal comma becomes a dot). The SQLite import runs **statement by statement**: a statement SQLite rejects is skipped and listed under "⚠ N Anweisung(en) übersprungen" instead of aborting the import. Result tables draw at most 500 rows (the row count stays exact). On open, the sandbox checks `/sql/filestatus` and **auto-loads `data/exam.sqlite`** if present (the fast exam-day path).
...
- The mysqldump→SQLite conversion (`sql-util.js`) is **best-effort** (it strips `ENGINE=`/`CHARSET`/non-unique `KEY` lines and `ON UPDATE CURRENT_TIMESTAMP`, turns an `AUTO_INCREMENT` single-column primary key into `INTEGER PRIMARY KEY AUTOINCREMENT` so id-less INSERTs work, folds phpMyAdmin/XAMPP's trailing `ALTER TABLE … ADD PRIMARY KEY/UNIQUE/CONSTRAINT … FOREIGN KEY` and `MODIFY … AUTO_INCREMENT` back into the `CREATE TABLE`, maps `INSERT IGNORE`→`INSERT OR IGNORE`, converts `ENUM`→`text`, and rewrites backslash-escaped string literals to SQLite doubling — so re-split converted text with `splitStatements(sql, { backslashEscapes: false })`). It exists only for the no-MySQL fallback; the accurate path is real local MySQL. Don't rely on it for exam-exact results.
