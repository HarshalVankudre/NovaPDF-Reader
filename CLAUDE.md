...
Four AI providers: `claude` (Haiku 4.5) and `sonnet` (Sonnet 4.6) go through the Anthropic SDK and share `ANTHROPIC_API_KEY`; `grok` and `deepseek` use an OpenAI-compatible `fetch`. Adding, removing, or renaming a provider means editing **all three**:
...
- It is driven from the keyboard: **Ctrl+Enter** asks the tutor; typed `:`-commands in the search box switch state (`:ai` toggles the visible controls, `:new` resets the thread, `:claude` / `:sonnet` / `:grok` / `:deepseek`, with `:haiku` as an alias for `claude`).
